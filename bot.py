...
def parse_steam_file(text: str):
    ...
    while i < len(lines):
        ...
        norm = (line
                .replace(" \u2013 ", "|").replace(" \u2014 ", "|")
                .replace(" - ", "|").replace(" | ", "|")
                .replace("GAMES:", "").replace("Games:", "").replace("games:", "")
                .strip())

        if "|" in norm and ":" in norm.split("|")[0]:
            parts = norm.split("|", 1)
            creds, games = parts[0].strip(), parts[1].strip()
        ...
        cred_index = next((j for j, bl in enumerate(block_lines) if is_credential_line(bl)), None)
...
def service_embed(service: str, user: str, pwd: str, games: str) -> discord.Embed:
    embed = discord.Embed(
    ...
    if games:
        embed.add_field(
            name="🎮 Games" if service == "steam" else "📋 Info",
            value=games[:1024] if len(games) <= 1024 else games[:1021] + "...",
            inline=False,
        )
...
# ================= /generate =================
@bot.tree.command(name="generate", description="Generate a Steam, Xbox, or Minecraft account")
...
async def generate(interaction: discord.Interaction, service: str, game: str = None):
    ...
    with db() as con:
        ...
        if game and service == "steam":
            cur.execute(
                f"SELECT id, username, password, games FROM {table} "
                f"WHERE games LIKE ? ORDER BY RANDOM() LIMIT 1",
                (f"%{game}%",),
            )
        else:
            cur.execute(
                f"SELECT id, username, password, games FROM {table} ORDER BY RANDOM() LIMIT 1"
            )
        row = cur.fetchone()
    ...
    embed = service_embed(service, user, pwd, games)
...
async def search(interaction: discord.Interaction, game: str):
    with db() as con:
        cur = con.cursor()
        cur.execute("SELECT COUNT(*) FROM steam_accounts WHERE games LIKE ?", (f"%{game}%",))
...
async def listgames(interaction: discord.Interaction):
    with db() as con:
        cur = con.cursor()
        cur.execute("SELECT DISTINCT games FROM steam_accounts")
        rows = cur.fetchall()

    games = sorted({g.strip() for (row,) in rows for g in row.split(",") if g.strip()})
...
# ================= STAFF COMMANDS =================
@bot.tree.command(name="restock", description="[Staff] Upload accounts to stock")
@app_commands.describe(service="Which service to restock", file="Text file of accounts")
@app_commands.choices(service=[app_commands.Choice(name=s.capitalize(), value=s) for s in SERVICES])
...
async def restock(interaction: discord.Interaction, service: str, file: discord.Attachment):
    ...
    if service == "steam":
        ...
        for line in text.splitlines():
            ...
            if ":" in line:
                ...
                if user.strip() and pwd.strip():
                    parsed.append((user.strip(), pwd.strip(), ""))
    ...
    with db() as con:
        cur = con.cursor()
        for user, pwd, games in parsed:
            cur.execute(f"INSERT INTO {table} (username, password, games) VALUES (?,?,?)", (user, pwd, games))
            added += 1
        con.commit()
...
async def globalstats(interaction: discord.Interaction):
    ...
    with db() as con:
        ...
        for svc in SERVICES:
            cur.execute(f"SELECT COUNT(*) FROM {svc}_accounts")
            total = cur.fetchone()[0]
            embed.add_field(name=f"{SERVICE_EMOJI[svc]} {svc.capitalize()}", value=f"**{total}** accounts", inline=True)
